...
lib.EmailsInput = (function(utils) {
  ...
  EmailsInput.prototype.getEmails = function getEmails() {
    const emailsList = [];
    document.querySelectorAll('.content').forEach(e => {
      if(!e.parentNode.classList.contains('invalid')) {
        emailsList.push(e.innerText);
      }
    })
  ...
  EmailsInput.prototype.remove = function remove(email) {
    console.log(this._inputContainerNode);
